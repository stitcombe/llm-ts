...
const TYPE_ERROR_MSG = {
    ...
    integer: "Input should be a valid integer, unable to parse string as an integer",
        ...
        for (const [name, def] of Object.entries(fields)) {
            ...
            if (name in data && data[name] !== undefined) {
                ...
                if (!ok) {
                    errors.push({
                        loc: [name],
                        msg: TYPE_ERROR_MSG[(Array.isArray(def.type) ? def.type[0] : def.type)],
                        type: "type_error",
                    });
                    continue;
                }
                ...
                if (def.enum && value !== null && !def.enum.includes(value)) {
                    ...
                    const quoted = def.enum.map((v) => JSON.stringify(v).replace(/"/g, "'"));
    ...
    *[Symbol.iterator]() {
    ...
    static modelJsonSchema() {
        ...
        for (const [name, def] of Object.entries(fields)) {
            ...
            const typeSchemas = jsonTypes.map((t) => {
                const s = { type: t };
...
export function createModel(name, fields, base = BaseModel) {
