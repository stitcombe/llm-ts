 ...
 */
function condenseString(value, replacements) {
    ...
    const matching = replacements.filter(([, text]) => text && value.includes(text));
    if (!matching.length) {
        return value;
    }
    const segments = [];
...
export function condenseJson(obj, replacements) {
...
export function uncondenseJson(obj, replacements) {
    function walk(value) {
        if (Array.isArray(value)) {
            return value.map((v) => walk(v));
        }
