...
embeddingsMigrations.register("m001_create_tables", (db) => {
    db.table("collections").create({ id: "int", name: "str", model: "str" }, { pk: "id" });
...
embeddingsMigrations.register("m004_store_content_hash", (db) => {
    ...
    db.conn.function("temp_md5", (text) => createHash("md5").update(String(text), "utf8").digest());
    db.conn.function("temp_random_md5", () => createHash("md5").update(String(Date.now() * Math.random()), "utf8").digest());
    db.execute(`
            update embeddings
