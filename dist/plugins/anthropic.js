...
import { AsyncKeyModel, KeyModel, Options as OptionsBase, ToolCall, } from "../models.js";
...
// ------------------------------------------------------------- options
const validateStopSequences = (stopSequences) => {
...
const validateWebSearchMaxUses = (maxUses) => {
    if (maxUses !== null && maxUses <= 0) {
        throw new Error("web_search_max_uses must be a positive integer");
    }
    return maxUses;
};
const validateWebSearchDomains = (domains) => {
    if (domains !== null) {
...
const validateWebSearchLocation = (location) => {
    if (location !== null) {
        ...
        if (!requiredFields.every((field) => field in location)) {
            throw new Error(`web_search_location must contain: {${requiredFields
                .map((f) => `'${f}'`)
                .join(", ")}}`);
        }
    }
...
export class ClaudeOptions extends OptionsBase {
    static fields = {
        ...
        top_k: {
            type: "integer",
            description: "Only sample from the top K options for each subsequent token. Used to " +
        ...
        fast: {
            type: "boolean",
            description: "Use fast mode for lower latency responses: " +
        ...
        web_search_location: {
            type: "object",
            description: "User location for localizing search results (dict with city, region, " +
                "country, timezone)",
            default: null,
        },
    };
...
export function sourceForAttachment(attachment) {
    if (attachment.url) {
        return { type: "url", url: attachment.url };
    }
    const prepared = attachment;
    return {
        data: prepared._base64 ??
 ...
 */
async function prepareAttachments(prompt) {
    for (const message of prompt.messages) {
        for (const part of message.parts) {
            if (!(part instanceof AttachmentPart) || !part.attachment)
                continue;
...
function initShared(self, modelId, init) {
    const { claude_model_id = null, supports_images = true, supports_pdf = false, supports_thinking = false, supports_thinking_effort = false, supports_adaptive_thinking = false, supports_web_search = false, use_structured_outputs = false, default_max_tokens = null, base_url = null, } = init;
...
function partToBlock(part) {
    ...
    const anthropicPm = (pm && typeof pm === "object" ? pm.anthropic : null) ?? {};
    ...
    if (part instanceof ReasoningPart) {
        ...
        const sig = anthropicPm && typeof anthropicPm === "object"
            ? anthropicPm.signature
            : null;
        if (sig) {
            block.signature = sig;
        }
    ...
    if (part instanceof AttachmentPart && part.attachment !== null) {
        const attachment = part.attachment;
        const attachmentType = (attachment.type ?? attachment.resolveTypeSync()) === "application/pdf"
            ? "document"
            : "image";
        return {
            type: attachmentType,
...
function messageToBlocks(message) {
    let blocks = [];
    for (const part of message.parts) {
 ...
 */
function appendMessage(out, message) {
...
function buildMessages(self, prompt) {
    const messages = [];
    const options = prompt.options;
    ...
    if (options.prefill) {
        if (self.supports_adaptive_thinking) {
            throw new Error(`Prefilling assistant messages is not supported by ${self.claude_model_id}. ` +
...
function buildKwargs(self, prompt) {
    const options = prompt.options;
    if (prompt.schema && prompt.tools.length) {
        throw new Error("llm-anthropic does not yet support using both schema and tools in the same prompt");
    }
    // Validate web search support
    if (options.web_search && !self.supports_web_search) {
        throw new Error(`Web search is not supported by model ${self.model_id}. ` +
    ...
    if (options.thinking_effort === "max" &&
        !self.claude_model_id.includes("opus")) {
        throw new Error(`thinking_effort='max' is only supported by the Claude Opus models, ` +
    ...
    const thinkingEffortEnabled = Boolean(self.supports_thinking_effort && options.thinking_effort);
    // Determine if thinking should be activated
    let thinkingRequested = false;
    if (self.supports_thinking) {
        thinkingRequested = Boolean(options.thinking ||
    ...
    const useStructuredOutputs = Boolean(prompt.schema && self.use_structured_outputs);
    ...
    if (prompt.tools.length) {
        tools.push(...prompt.tools.map((tool) => ({
            name: tool.name,
...
function setUsage(response) {
    ...
    let details = null;
    if (response.prompt.options.cache || usage.server_tool_use) {
        details = usage;
    }
    response.set_usage({
        input: inputTokens,
...
function addToolUsage(response, lastMessage) {
    const toolUses = lastMessage.content.filter((item) => item.type === "tool_use");
...
function* nonStreamingEvents(completion, initialPrefill) {
 ...
 */
async function* streamingEvents(self, client, kwargs, response, prefill, emitTrailingSpace) {
    void self;
...
async function* execute(self, prompt, stream, response, key, emitTrailingSpace) {
    ...
    if (stream) {
        yield* streamingEvents(self, client, kwargs, response, prefill, emitTrailingSpace);
    }
    else {
...
// -------------------------------------------------------------- models
export class ClaudeMessages extends KeyModel {
    needs_key = "anthropic";
    ...
    static Options = ClaudeOptions;
    constructor(modelId, init = {}) {
        super();
        initShared(this, modelId, init);
    }
    build_messages(prompt, conversation) {
        void conversation;
        return buildMessages(this, prompt);
    }
    build_kwargs(prompt, conversation) {
        void conversation;
        return buildKwargs(this, prompt);
    }
    _extract_system(prompt) {
...
export class AsyncClaudeMessages extends AsyncKeyModel {
    needs_key = "anthropic";
    key_env_var = "ANTHROPIC_API_KEY";
    can_stream = true;
    base_url = null;
...
export const register_models = hookimpl(function register_models(register) {
    for (const spec of MODEL_SPECS) {
        register(new ClaudeMessages(spec.id, spec.init ?? {}), new AsyncClaudeMessages(spec.id, spec.init ?? {}), spec.aliases ?? null);
    }
});
