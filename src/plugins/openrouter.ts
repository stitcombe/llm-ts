...
import type { FieldDef, Validator } from "../pydantic.js";
import { Group, Option, echo } from "../click/index.js";
...
export async function fetchCachedJson(
  ...
  try {
    ...
    throw new DownloadError(
      `Failed to download data and no cache is available at ${cachePath}: ${
        (e as Error).message
      }`,
    );
  }
...
export async function ensureModelsCached(skipCache = false): Promise<void> {
  ...
  await fetchCachedJson(
    MODELS_URL,
    modelsCachePath(),
    skipCache ? 0 : CACHE_TIMEOUT_SECONDS,
  );
}

/** Sync half: read whatever ensureModelsCached() last wrote. */
export function getOpenrouterModels(): Json[] {
  const cachePath = modelsCachePath();
  if (!fs.existsSync(cachePath)) {
    return [];
  }
  return (JSON.parse(fs.readFileSync(cachePath, "utf-8")).data ?? []) as Json[];
}

export function getModelIds(): string[] {
  return getOpenrouterModels().map((model) => model.id as string);
}

export function getSupportsImages(modelDefinition: Json): boolean {
  const modalities = modelDefinition?.architecture?.input_modalities;
  return Array.isArray(modalities) && modalities.includes("image");
}

export function hasParameter(modelDefinition: Json, parameter: string): boolean {
...
const validateProvider: Validator = (provider) => {
  if (provider === null) {
    return null;
  }
...
const MIXIN_FIELDS: Record<string, FieldDef> = {
  online: {
    type: "boolean",
  ...
  reasoning_effort: {
    type: "string",
    description:
      'One of "high", "medium", or "low" to control reasoning effort',
    default: null,
...
const MIXIN_VALIDATORS: Record<string, Validator> = {
 ...
 */
function openrouterBuildKwargs(base: Json, prompt: Prompt): Json {
  const kwargs = { ...base };
...
function optionsClassFor(base: typeof Chat | typeof AsyncChat): any {
  const cls = class extends (base.Options as any) {
...
export class OpenRouterChat extends Chat {
  override needs_key: string | null = "openrouter";
  override key_env_var: string | null = "OPENROUTER_KEY";

  static override Options = optionsClassFor(Chat);

  override build_kwargs(prompt: Prompt, stream: boolean): Json {
...
export class OpenRouterAsyncChat extends AsyncChat {
  ...
  static override Options = optionsClassFor(AsyncChat);
...
export const register_models = hookimpl(function register_models(
  ...
  for (const modelDefinition of getOpenrouterModels()) {
    const init = {
      ...
      model_name: modelDefinition.id as string,
    ...
    register(
      new OpenRouterChat(init) as unknown as Model,
      new OpenRouterAsyncChat(init) as unknown as AsyncModel,
    );
  }
});
...
export function formatPrice(key: string, priceStr: string): string | null {
...
export function formatPricing(pricingDict: Json): string {
  const parts: string[] = [];
  for (const [key, value] of Object.entries(pricingDict)) {
    const formatted = formatPrice(key, value as string);
    if (formatted) {
...
export const register_commands = hookimpl(function register_commands(
  ...
  openrouter.command({
    ...
    handler: async (params) => {
      ...
      const free = params.free as boolean;
      const jsonOut = params.json_ as boolean;
      const allModels = free
        ? getOpenrouterModels().filter((model) =>
            (model.id as string).endsWith(":free"),
          )
        : getOpenrouterModels();
      if (jsonOut) {
      ...
      for (const model of allModels) {
        ...
        bits.push(
          `  context_length: ${Number(model.context_length).toLocaleString("en-US")}`,
        );
        ...
        bits.push(
          `  supports_schema: ${pyBool(hasParameter(model, "structured_outputs"))}`,
        );
  ...
  openrouter.command({
    ...
    handler: async () => {
      ...
      if (added.length) {
        echo(
          `Added models: ${added.map((m) => "openrouter/" + m).join(", ")}`,
          { err: true },
        );
      }
      if (removed.length) {
        echo(
          `Removed models: ${removed.map((m) => "openrouter/" + m).join(", ")}`,
          { err: true },
        );
      } else {
