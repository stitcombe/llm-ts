...
import DatabaseConstructor, { type Database as BetterSqlite3 } from "better-sqlite3";
import { dumps } from "./pyjson.js";

export type ColumnType = "str" | "int" | "float" | "bytes";

const TYPE_TO_SQL: Record<ColumnType, string> = {
  str: "TEXT",
  int: "INTEGER",
  float: "FLOAT",
  bytes: "BLOB",
...
function sqlTypeToToken(sqlType: string): ColumnType {
  const t = sqlType.toUpperCase();
  if (t.includes("INT")) return "int";
  if (t.includes("BLOB") || t === "") return "bytes";
  if (
    t.includes("REAL") ||
    t.includes("FLOA") ||
    t.includes("DOUB")
...
export class Database {
  ...
  constructor(pathOrOptions: string | { memory: boolean } = { memory: true }) {
    const path =
      typeof pathOrOptions === "string" ? pathOrOptions : ":memory:";
  ...
  query(
    ...
    if (cleaned === undefined) return stmt.all() as Array<Record<string, unknown>>;
    return stmt.all(cleaned as never) as Array<Record<string, unknown>>;
  }

  execute(
    sql: string,
    params?: unknown[] | Record<string, unknown>,
  ): { rows: Array<Record<string, unknown>>; lastInsertRowid: number | bigint } {
    const stmt = this.conn.prepare(sql);
    const cleaned = sanitizeParams(params);
    if (stmt.reader) {
      const rows =
        cleaned === undefined
          ...
          : (stmt.all(cleaned as never) as Array<Record<string, unknown>>);
      return { rows, lastInsertRowid: -1 };
    }
    const info =
      cleaned === undefined ? stmt.run() : stmt.run(cleaned as never);
  ...
  get triggers(): Trigger[] {
    return this.query(
      "select name, tbl_name as [table], sql from sqlite_master where type = 'trigger'",
    ).map((r) => ({
  ...
  registerFunction(
    ...
    this.conn.function(name, { deterministic: true }, fn as never);
  }
...
export class Table {
  ...
  name: string;
  lastPk: unknown = null;
  lastRowid: number | bigint | null = null;
  ...
  get schema(): string {
    ...
    return rows.length ? (rows[0].sql as string) : "";
  }
  ...
  countWhere(
    ...
    return this.db.query(sql, params)[0].c as number;
  }
  ...
  get foreignKeys(): ForeignKey[] {
    return this.db
      ...
      .map((r) => ({
        ...
        other_table: r.table as string,
        other_column: (r.to as string) ?? "id",
      }));
  }

  get triggers(): Trigger[] {
    return this.db.query(
      "select name, tbl_name as [table], sql from sqlite_master where type = 'trigger' and tbl_name = ?",
      [this.name],
  ...
  create(
    ...
    { pk, foreignKeys = [], ifNotExists = false }: CreateOptions = {},
  ): Table {
    ...
    const sql = `CREATE TABLE ${ifNotExists ? "IF NOT EXISTS " : ""}${q(
      this.name,
    )} (\n${colDefs.join(",\n")}\n)`;
  ...
  get(pkValues: unknown): Record<string, unknown> {
    ...
    if (pks.length !== values.length) {
      throw new NotFoundError(
        `Need ${pks.length} primary key value${pks.length === 1 ? "" : "s"}`,
      );
    }
  ...
  insertAll(
    ...
    { pk, ignore = false, replace = false }: InsertOptions = {},
  ): Table {
    ...
    for (const record of records) {
      ...
      const info = this.db.conn
        .prepare(sql)
        .run(...keys.map((k) => toSqlValue(record[k]) as never));
  ...
  deleteWhere(
    ...
    const cleaned = sanitizeParams(params);
    if (cleaned === undefined) stmt.run();
    else stmt.run(cleaned as never);
    return this;
  }
  ...
  addColumn(
    name: string,
    type: ColumnType = "str",
    { fk, fkCol }: { fk?: string; fkCol?: string } = {},
  ): Table {
    let sql = `ALTER TABLE ${q(this.name)} ADD COLUMN ${q(name)} ${
      TYPE_TO_SQL[type]
    }`;
    if (fk) {
  ...
  createIndex(
    columns: string[],
    { unique = false, ifNotExists = false }: { unique?: boolean; ifNotExists?: boolean } = {},
  ): Table {
    const indexName = `idx_${this.name}_${columns.join("_")}`;
    this.db.conn.exec(
      `CREATE ${unique ? "UNIQUE " : ""}INDEX ${
        ifNotExists ? "IF NOT EXISTS " : ""
      }${q(indexName)} ON ${q(this.name)} (${columns.map(q).join(", ")})`,
    );
    return this;
  }
  ...
  transform(
    ...
    addForeignKeys: Array<[string, string, string]> = [],
  ): Table {
    ...
    const newFks: Array<[string, string, string]> = [];
    for (const fk of oldFks) {
      if (dropFkSet.has(fk.column)) continue;
      const renamed = rename[fk.column] ?? fk.column;
      if (dropSet.has(fk.column)) continue;
    ...
    for (const col of cols) {
      ...
      if (singlePk === col.newName) def += " PRIMARY KEY";
      const fk = newFks.find(([c]) => c === col.newName);
      if (fk) def += ` REFERENCES ${q(fk[1])}(${q(fk[2])})`;
      colDefs.push(def);
    }
    ...
    try {
      ...
      this.db.conn.exec(
        `INSERT INTO ${q(tempName)} (${insertCols}) SELECT ${selectCols} FROM ${q(
          this.name,
        )}`,
      );
      this.db.conn.exec(`DROP TABLE ${q(this.name)}`);
      this.db.conn.exec(
        `ALTER TABLE ${q(tempName)} RENAME TO ${q(this.name)}`,
      );
    } finally {
      if (wasOn) this.db.conn.pragma("foreign_keys = on");
    }
    return this;
  }
  ...
  enableFts(
    columns: string[],
    {
      ...
      replace = false,
    }: { createTriggers?: boolean; replace?: boolean } = {},
  ): Table {
    ...
    if (replace || !this.db.table(ftsName).exists()) {
      ...
      for (const suffix of ["ai", "ad", "au"]) {
        this.db.conn.exec(
          `DROP TRIGGER IF EXISTS ${q(`${this.name}_${suffix}`)}`,
        );
      }
      this.db.conn.exec(
        `CREATE VIRTUAL TABLE ${q(ftsName)} USING FTS5 (${columns
          .map(q)
          .join(", ")}, content=${q(this.name)})`,
      );
      // Populate with existing content
      this.db.conn.exec(
        `INSERT INTO ${q(ftsName)} (rowid, ${columns.map(q).join(", ")}) ` +
  ...
  search(query: string): Array<Record<string, unknown>> {
    const ftsName = `${this.name}_fts`;
    return this.db.query(
      `select rowid from ${q(ftsName)} where ${q(ftsName)} match ?`,
      [query],
    );
  }
}
