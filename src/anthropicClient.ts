 ...
 */
export function normalizeMessage(message: Json): Json {
  const out: Json = { ...message };
  if (!("container" in out)) out.container = null;
  if (!("stop_details" in out)) out.stop_details = null;
  out.content = (out.content ?? []).map((block: Json) => {
    const b: Json = { ...block };
    if (b.type === "text") {
      if (!("citations" in b)) b.citations = null;
      if (!("parsed_output" in b)) b.parsed_output = null;
    }
    return b;
  });
  return out;
...
class MessageAccumulator {
  ...
  private blocks = new Map<number, Json>();
  private jsonBuffers = new Map<number, string>();
  ...
  add(event: Json): void {
    switch (event.type) {
      ...
      case "content_block_start": {
        const block: Json = { ...(event.content_block ?? {}) };
      ...
      case "content_block_delta": {
        ...
        if (delta.type === "text_delta") {
          ...
          this.jsonBuffers.set(
            event.index,
            (this.jsonBuffers.get(event.index) ?? "") + delta.partial_json,
          );
        }
        break;
      }
...
export class AnthropicClient {
  ...
  constructor({ apiKey, baseUrl = null }: AnthropicClientOptions) {
    this.apiKey = apiKey;
   ...
   */
  private prepare(params: Json): { body: Json; betas: string[] } {
    const { betas, extra_body: extraBody, ...rest } = params;
    const body: Json = { ...rest, ...(extraBody ?? {}) };
    return { body, betas: (betas as string[] | undefined) ?? [] };
  }
  ...
  private async request(params: Json, stream: boolean): Promise<globalThis.Response> {
  ...
  messages = {
    create: async (params: Json): Promise<Json> => {
    ...
    stream: (params: Json): MessageStream => {
      ...
      async function* iterate(): AsyncGenerator<Json> {
        ...
        let buffer = "";
        const emit = function* (rawEvent: string): Generator<Json> {
