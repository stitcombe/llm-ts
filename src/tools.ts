...
export function llm_version(): string {
  // "Return the installed version of llm"
  const require = createRequire(import.meta.url);
  const pkg = require("../package.json") as { version: string };
  return pkg.version;
}
(llm_version as { description?: string }).description =
  "Return the installed version of llm";
...
export function llm_time(): Record<string, unknown> {
  ...
  const timezoneOffset = `UTC${offsetHours >= 0 ? "+" : ""}${String(
    offsetHours,
  ).padStart(2, "0")}:${pad(offsetMinutes)}`;
...
(llm_time as { description?: string }).description =
  "Returns the current time, as local time and UTC";
