...
import {
  ...
  getEmbeddingModelsWithAliases,
  getFragmentLoaders,
  getModel,
  getModelAliases,
  getModelsWithAliases,
  getPlugins,
  getTemplateLoaders,
  getTools,
  removeAlias,
...
import { Database, NotFoundError, Table } from "./sqliteUtils.js";
...
import { BaseModel, ValidationError } from "./pydantic.js";
...
export async function displayStreamEvents(
  events: AsyncIterable<StreamEvent> | Iterable<StreamEvent>,
  { showReasoning = true }: { showReasoning?: boolean } = {},
): Promise<void> {
...
function validateFragmentAlias(
  _ctx: Context,
  _param: Argument,
  value: unknown,
): unknown {
...
): Promise<Array<Fragment | Attachment>> {
  function loadByAlias(fragment: string): [string | null, string | null] {
    const rows = db.query(
      `
    ...
    if (rows.length) {
      return [rows[0].content as string, rows[0].source as string | null];
    }
    return [null, null];
  }

  const resolved: Array<Fragment | Attachment> = [];
    ...
    if (fragment.startsWith("http://") || fragment.startsWith("https://")) {
      ...
      if (!response.ok) {
        throw new FragmentNotFound(
          `Could not load fragment ${fragment}: HTTP ${response.status}`,
        );
      }
      ...
      try {
        ...
        if (
          ...
          throw new FragmentNotFound(
            `Fragment loader ${prefix} returned a disallowed attachment`,
          );
        }
        resolved.push(...result);
      } catch (ex) {
        throw new FragmentNotFound(
          `Could not load fragment ${fragment}: ${(ex as Error).message}`,
        );
      }
    } else {
      // Try from the DB
...
async function processFragmentsInChat(
  db: Database,
  prompt: string,
): Promise<[string, Fragment[], Attachment[]]> {
    ...
    if (line.startsWith("!fragment ")) {
      try {
        const fragmentStrs = line
          ...
          .replace(/^!fragment /, "")
          .split(/\s+/);
        const fragmentsAndAttachments = await resolveFragments(
          db,
          fragmentStrs,
          true,
        );
        fragments.push(
          ...fragmentsAndAttachments.filter(
            (f): f is Fragment => f instanceof Fragment,
          ),
        );
        attachments.push(
          ...fragmentsAndAttachments.filter(
            (a): a is Attachment => a instanceof Attachment,
          ),
        );
      } catch (ex) {
...
export async function resolveAttachment(value: string): Promise<Attachment> {
  if (value === "-") {
  ...
  if (value.includes("://")) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Client error '${response.status}' for url '${value}'`,
        );
      }
      mimetype = response.headers.get("content-type");
    } catch (ex) {
      throw new AttachmentError((ex as Error).message);
    }
...
function resolveAttachmentWithType(value: string, mimetype: string): Attachment {
  if (value.includes("://")) {
...
function jsonValidator(objectName: string) {
  return (_ctx: Context, _param: Option, value: unknown): unknown => {
    ...
    try {
      ...
      if (e instanceof click.BadParameter) throw e;
...
function humanReadableSize(sizeBytes: number): string {
...
export function logsOn(): boolean {
  return !fs.existsSync(path.join(userDir(), "logs-off"));
}

export function getAllModelOptions(): Record<string, Record<string, unknown>> {
...
export function getModelOptions(modelId: string): Record<string, unknown> {
  return getAllModelOptions()[modelId] ?? {};
}

export function setModelOption(
  modelId: string,
  key: string,
  value: unknown,
): void {
...
export function clearModelOption(modelId: string, key: string): void {
  const p = path.join(userDir(), "model_options.json");
  if (!fs.existsSync(p)) return;
  let options: Record<string, Record<string, unknown>>;
  try {
...
function parseYamlTemplate(name: string, content: string): Template {
  let loaded: unknown;
  try {
    loaded = yaml.load(content);
  } catch (ex) {
    throw new LoadTemplateError(`Invalid YAML: ${(ex as Error).message}`);
  }
  if (typeof loaded === "string") {
    return new Template({ name, prompt: loaded });
  }
  if (loaded && typeof loaded === "object" && !Array.isArray(loaded)) {
    fixClipChomping(loaded as Record<string, unknown>, content);
  }
  const data = { ...(loaded as Record<string, unknown>), name };
  try {
...
export async function loadTemplate(name: string): Promise<Template> {
  if (name.startsWith("https://") || name.startsWith("http://")) {
    ...
    if (!response.ok) {
      throw new LoadTemplateError(
        `Could not load template ${name}: HTTP ${response.status}`,
      );
    }
  ...
  if (hasPluginPrefix(name) && !fs.existsSync(name)) {
    ...
    try {
      ...
      throw new LoadTemplateError(
        `Could not load template ${name}: ${(ex as Error).message}`,
      );
    }
  }
 ...
 */
function toolsFromCode(codeOrPath: string): Tool[] {
  let code = codeOrPath;
  if (
    !codeOrPath.includes("\n") &&
  ...
  try {
    ...
    namespace = factory() as Record<string, unknown>;
  } catch (ex) {
    throw new ClickException(
      `Error in --functions definition: ${(ex as Error).message}`,
    );
  }
  for (const [name, value] of Object.entries(namespace)) {
    if (typeof value === "function" && !name.startsWith("_")) {
      tools.push(Tool.function(value as (...args: unknown[]) => unknown));
    }
  }
  return tools;
}

function debugToolCall(
  _tool: Tool | null,
  toolCall: ToolCall,
  ...
  if (toolResult.exception) {
    echo(
      style(`  Exception: ${toolResult.exception.message}`, {
        fg: "red",
        bold: true,
      }),
      { err: true },
...
function approveToolCall(_tool: Tool | null, toolCall: ToolCall): void {
  echo(
    style(`Tool call: ${toolCall.name}(${dumps(toolCall.arguments)})`, {
      fg: "yellow",
      bold: true,
    }),
    { err: true },
...
function gatherTools(
  ...
  const registeredClasses: Record<string, new (...args: unknown[]) => unknown> =
    {};
  for (const [key, value] of Object.entries(registeredTools)) {
    if (typeof value === "function") {
      registeredClasses[key] = value as unknown as new (
        ...args: unknown[]
      ) => unknown;
    }
  }
  const badTools = toolSpecs.filter(
    (tool) => !(tool.split("(")[0] in registeredTools),
  );
  if (badTools.length) {
    throw new ClickException(
      `Tool(s) ${badTools.join(", ")} not found. Available tools: ${Object.keys(
        registeredTools,
      ).join(", ")}`,
    );
  }
  for (const toolSpec of toolSpecs) {
    if (toolSpec[0] !== toolSpec[0].toUpperCase()) {
      // It's a function
      tools.push(registeredTools[toolSpec] as ToolDef);
    } else {
      // It's a class
      tools.push(
        instantiateFromSpec(
          registeredClasses as Record<string, new (...args: any[]) => any>,
          toolSpec,
        ) as ToolDef,
      );
    }
...
export async function loadConversation(
...
): Promise<_BaseConversation | null> {
  ...
  for (const responseRow of db
    ...
    conversation.responses.push(responseObj as never);
  }
  return conversation;
}

function resolveSchemaInput(
  db: Database,
  schemaInput: string | null,
  loadTemplateFn: (name: string) => Promise<Template>,
...
function modelMatchesIdOrAlias(
  modelWithAliases: {
...
function renderModelWithAliases(
  modelWithAliases: any,
  {
    options = false,
    async_ = false,
    ...
    options?: boolean;
    async_?: boolean;
    modelsThatHaveShownOptions?: Set<unknown> | null;
  } = {},
): string {
  ...
  const optionsSchema = (model.Options as typeof BaseModel).modelJsonSchema();
  const properties = optionsSchema.properties as Record<
    string,
  ...
  if (options && Object.keys(properties).length) {
    ...
    for (const [name, field] of Object.entries(properties)) {
      let anyOf = field.anyOf as Array<Record<string, unknown>> | undefined;
      if (anyOf === undefined) {
      ...
      const types = anyOf
        ...
        .map(
          (item) =>
            TYPE_LOOKUP[item.type as string] ?? (item.type as string) ?? "str",
        )
        .join(", ");
      const bits = ["\n    ", name, ": ", types];
...
function renderModelWithOptions(
  modelId: string,
  { async_ = false }: { async_?: boolean } = {},
): string {
...
function displayTruncated(text: string): string {
...
function optionsFromValidated(
  model: { Options: any },
  options: Array<[string, string]>,
): Record<string, unknown> {
  const validated: Record<string, unknown> = {};
  try {
    const optionsModel = new model.Options(
...
function pyYamlDump(data: unknown, opts: Record<string, unknown> = {}): string {
  // js-yaml dump with settings closest to PyYAML's defaults
  return yaml.dump(data, {
    noRefs: true,
    lineWidth: (opts.width as number) ?? 80,
    sortKeys: (opts.sortKeys as boolean) ?? true,
    quotingType: "'",
    forceQuotes: false,
  ...
  new Command({
    ...
    handler: async (params) => {
      let {
        ...
        attachment_types: attachmentTypePairs,
        tools,
        python_tools,
        tools_debug,
        tools_approve,
      ...
      let attachment_types: Attachment[] = (
        attachmentTypePairs as Array<[string, string]>
      ).map(([value, mimetype]) => resolveAttachmentWithType(value, mimetype));

      if ((queries as string[]).length && !model_id) {
        // Use -q options to find model with shortest model_id
        const matches: string[] = [];
        for (const modelWithAliases of getModelsWithAliases()) {
          if (
            (queries as string[]).every((q) => modelWithAliases.matches(q))
          ) {
        ...
        if (!matches.length) {
          throw new ClickException(
            `No model found matching queries ${(queries as string[]).join(", ")}`,
          );
        }
      ...
      if (save) {
        ...
        if (disallowedOptions.length) {
          throw new ClickException(
            `--save cannot be used with ${disallowedOptions.join(", ")}`,
          );
        }
        ...
        prompt = readPrompt();
        if (prompt) toSave.prompt = prompt;
        if (system) toSave.system = system;
        if ((param as Array<[string, string]>).length) {
          toSave.defaults = Object.fromEntries(param as Array<[string, string]>);
        }
        if (extract) toSave.extract = true;
        if (extract_last) toSave.extract_last = true;
        if (schema) toSave.schema_object = schema;
        if ((fragments as string[]).length) toSave.fragments = [...fragments];
        if ((system_fragments as string[]).length) {
          toSave.system_fragments = [...system_fragments];
        }
        if ((python_tools as string[]).length) {
          toSave.functions = (python_tools as string[]).join("\n\n");
        }
        if ((tools as string[]).length) toSave.tools = [...tools];
        ...
        if ((options as Array<[string, string]>).length) {
          const model = getModel(model_id || getDefaultModel());
          try {
            const optionsModel = new (model.Options as any)(
              Object.fromEntries(options as Array<[string, string]>),
            ) as BaseModel;
            const dumped: Record<string, unknown> = {};
            for (const [k, v] of Object.entries(optionsModel.modelDump())) {
              if (v !== null && v !== undefined) dumped[k] = v;
            }
        ...
        fs.writeFileSync(
          savePath,
          pyYamlDump(toSave, { sortKeys: false, indent: 4 }),
        );
        return;
      }

      let templateObj: Template | null = null;
      if (template) {
        const params_ = Object.fromEntries(param as Array<[string, string]>);
        try {
        ...
        if (templateObj.options) {
          options = [...options];
          const specifiedOptions = Object.fromEntries(
          ...
          for (const [optionName, optionValue] of Object.entries(
            ...
            if (!(optionName in specifiedOptions)) {
              (options as Array<[string, unknown]>).push([
                optionName,
                optionValue,
              ]);
            }
        ...
        try {
          const [templatePrompt, templateSystem] = templateObj.evaluate(
            input_,
            params_,
          );
          if (templatePrompt) {
        ...
        if (templateObj.attachment_types) {
          attachment_types = [
            ...templateObj.attachment_types.map((at: any) =>
              resolveAttachmentWithType(at.value, at.type),
            ),
            ...attachment_types,
          ];
        }
      ...
      let conversation: _BaseConversation | null = null;
      ...
      if (
        conversation === null &&
        ((tools as string[]).length || (python_tools as string[]).length)
      ) {
      ...
      // Validate options
      let validatedOptions: Record<string, unknown> = {};
      if ((options as Array<[string, string]>).length) {
      ...
      let resolvedFragments: Fragment[];
      let resolvedSystemFragments: Array<Fragment | Attachment>;
      try {
        const fragmentsAndAttachments = await resolveFragments(
          db,
          fragments,
        ...
        resolvedFragments = fragmentsAndAttachments.filter(
          (f): f is Fragment => f instanceof Fragment,
        );
        resolvedAttachments = [
          ...resolvedAttachments,
      ...
      let promptTarget: any = model;
      let promptMethodName: "prompt" | "chain" = "prompt";
      if (conversation) {
      ...
      try {
        ...
        if (shouldStream) {
          const events =
            typeof response.astream_events === "function"
          ...
          let text: string =
            typeof response.textAsync === "function"
        ...
        if (testState.calledFromTest || process.env.LLM_RAISE_ERRORS) {
          ...
          if (
            ...
            throw new ClickException((ex as Error).message);
          }
          throw ex;
        }
        throw new ClickException((ex as Error).message);
      ...
      if (usage) {
        const responses =
          response instanceof _BaseChainResponse
            ? response._responses
            : [response];
        for (const responseObject of responses) {
          echo(
            style(`Token usage: ${responseObject.token_usage()}`, {
              fg: "yellow",
              bold: true,
            }),
            { err: true },
...
cli.addCommand(
  new Command({
    ...
    handler: async (params) => {
      ...
      if (template) {
        params_ = Object.fromEntries(param as Array<[string, string]>);
        try {
          templateObj = await loadTemplate(template);
        } catch (ex) {
          if (ex instanceof LoadTemplateError) {
      ...
      // Validate options
      let validatedOptions: Record<string, unknown> = getModelOptions(
        model.model_id,
      );
      if ((options as Array<[string, string]>).length) {
        validatedOptions = optionsFromValidated(model, options);
      ...
      let argumentFragments: Fragment[];
      let argumentAttachments: Attachment[];
      let argumentSystemFragments: Array<Fragment | Attachment>;
      try {
        const fragmentsAndAttachments = await resolveFragments(
          db,
          fragments,
        ...
        argumentFragments = fragmentsAndAttachments.filter(
          (f): f is Fragment => f instanceof Fragment,
        );
        argumentAttachments = fragmentsAndAttachments.filter(
          (a): a is Attachment => a instanceof Attachment,
        );
      ...
      let accumulatedFragments: Fragment[] = [];
      let accumulatedAttachments: Attachment[] = [];
      let endToken = "!end";
      while (true) {
        ...
        let prompt: string = line;
        let fragments_: Fragment[] = [];
        let attachments: Attachment[] = [];
        ...
        if (prompt.trim().startsWith("!fragment ")) {
          const [newPrompt, newFragments, newAttachments] =
            await processFragmentsInChat(db, prompt);
          prompt = newPrompt;
        ...
        if (templateObj) {
          let templatePrompt: string | null;
          let templateSystem: string | null;
          const usesInput = templateObj.vars().has("input");
          try {
            const input_ = usesInput ? prompt : "";
            [templatePrompt, templateSystem] = templateObj.evaluate(
              input_,
              params_,
            );
          } catch (ex) {
        ...
        const response = conversation.chain(prompt, {
          ...
          system_fragments: argumentSystemFragments.filter(
            (f): f is Fragment => f instanceof Fragment,
          ),
          attachments,
          system,
...
keysGroup.command({
  ...
  handler: async (params) => {
    let value = params.value as string | null;
    if (!value) {
    ...
    current[params.name as string] = value;
...
logsGroup.command({
  ...
  handler: async () => {
    ...
    echo(
      `Database file size: \t\t${humanReadableSize(fs.statSync(p).size)}`,
    );
  },
});
...
logsGroup.command({
  ...
  handler: async (params) => {
    ...
    echo(
      `Backed up ${humanReadableSize(fs.statSync(targetPath).size)} to ${targetPath}`,
    );
  },
});
...
async function logsListHandler(params: Record<string, any>): Promise<void> {
  let {
    count,
    path: pathOption,
    database,
  ...
  const sqlFormat: Record<string, string> = {
    limit,
  ...
  if ((fragments as string[]).length) {
    const fragmentHashes = (await resolveFragments(db, fragments)).map((f) =>
      (f as Fragment).id(),
    );
  ...
  const attachmentsRows = ids.length
    ...
    : [];
  const attachmentsById: Record<string, Array<Record<string, unknown>>> = {};
  for (const attachment of attachmentsRows) {
    (attachmentsById[attachment.response_id as string] ??= []).push(attachment);
  }

  const FRAGMENTS_SQL = (table: string, placeholders: string) => `
    select
        ${table}.response_id,
        fragments.hash,
  ...
  const promptFragmentsById: Record<string, Array<Record<string, unknown>>> =
    {};
  const systemFragmentsById: Record<string, Array<Record<string, unknown>>> =
    {};
  for (const [table, dictionary] of [
    ["prompt_fragments", promptFragmentsById],
    ["system_fragments", systemFragmentsById],
  ] as Array<[string, Record<string, Array<Record<string, unknown>>>]>) {
    if (!ids.length) continue;
    for (const fragment of db.query(
      FRAGMENTS_SQL(table, ids.map(() => "?").join(",")),
      ids,
    )) {
      (dictionary[fragment.response_id as string] ??= []).push(fragment);
    }
  }

  if (data || data_array || data_key || data_ids) {
    const toOutput: Array<Record<string, unknown>> = [];
    for (const row of rows) {
      const responseText = (row.response as string) || "";
      try {
        ...
        if (
          ...
          (decoded[data_key] as unknown[]).every(
  ...
  const TOOLS_SQL = (placeholders: string) => `
    SELECT responses.id,
    COALESCE(
    ...
    `;
  const toolInfoById: Record<string, Record<string, unknown>> = {};
  if (ids.length) {
    for (const row of db.query(
      TOOLS_SQL(ids.map(() => "?").join(",")),
      ids,
    )) {
      toolInfoById[row.id as string] = {
  ...
  for (const row of rows) {
    if (truncate) {
      row.prompt = truncateString((row.prompt as string) || "");
      row.response = truncateString((row.response as string) || "");
    }
    for (const key of ["prompt_fragments", "system_fragments"]) {
      const source =
        key === "prompt_fragments"
          ? (promptFragmentsById[row.id as string] ?? [])
          : (systemFragmentsById[row.id as string] ?? []);
      row[key] = source.map((fragment) => ({
        hash: fragment.hash,
        content: expand
          ? fragment.content
          : truncateString(fragment.content as string),
        aliases: JSON.parse(fragment.aliases as string),
      }));
    }
    ...
    Object.assign(row, toolInfoById[row.id as string] ?? {
      tools: [],
      tool_calls: [],
      tool_results: [],
    });
  ...
  if (json_output) {
    for (const row of rows) {
      row.attachments = (attachmentsById[row.id as string] ?? []).map(
        (attachment) =>
          Object.fromEntries(
            Object.entries(attachment).filter(([k]) => k !== "response_id"),
          ),
      );
    }
    ...
    if (rows.length) {
      output = rows[rows.length - 1].response as string;
    }
  }
...
function renderLogsHumanReadable(
  rows: Array<Record<string, unknown>>,
  {
    ...
    attachmentsById: Record<string, Array<Record<string, unknown>>>;
  },
): void {
  function displayFragments(
    ...
    if (!expand) {
      ...
      content = fragments
        .map(
          (f) =>
            `<details><summary>${f.hash}</summary>\n${maybeFencedCode(
              f.content as string,
            )}\n</details>`,
        )
        .join("\n");
  ...
  for (const row of rows) {
    if (short) {
      const system = truncateString((row.system as string) || "", 120, true);
      ...
      const attachments = attachmentsById[row.id as string];
      const obj: Record<string, unknown> = {
        model: row.model,
        datetime: (row.datetime_utc as string).split(".")[0],
        conversation: cid,
      };
      if ((row.tool_calls as Array<Record<string, unknown>>).length) {
        obj.tool_calls = (row.tool_calls as Array<Record<string, unknown>>).map(
          (toolCall) => `${toolCall.name}(${dumps(toolCall.arguments)})`,
        );
      }
      if ((row.tool_results as Array<Record<string, unknown>>).length) {
        obj.tool_results = (
          row.tool_results as Array<Record<string, unknown>>
        ).map(
          (toolResult) =>
            `${toolResult.name}: ${truncateString(toolResult.output as string)}`,
        );
      }
      ...
      if (attachments) {
        obj.attachments = attachments.map((attachment) => {
          const details: Record<string, unknown> = { type: attachment.type };
          if (attachment.path) details.path = attachment.path;
          if (attachment.url) details.url = attachment.url;
          return details;
        });
      }
      for (const key of ["prompt_fragments", "system_fragments"]) {
        obj[key] = (row[key] as Array<Record<string, unknown>>).map(
          (f) => f.hash,
        );
      }
      if (usage && (row.input_tokens || row.output_tokens)) {
        ...
        if (row.token_details) {
          usageDetails.details = JSON.parse(row.token_details as string);
        }
    ...
    echo(
      `# ${(row.datetime_utc as string).split(".")[0]}${
    ...
    echo(`## Prompt\n\n${row.prompt || "-- none --"}`);
    displayFragments(
      row.prompt_fragments as Array<Record<string, unknown>>,
      "Prompt fragments",
    );
    if (row.options_json) {
      ...
      if (options && Object.keys(options as object).length) {
        const optionsText = Object.entries(options as Record<string, unknown>)
    ...
    if (row.system !== currentSystem) {
      ...
      currentSystem = row.system as string | null;
    }
    displayFragments(
      row.system_fragments as Array<Record<string, unknown>>,
      "System fragments",
    );
    if (row.schema_json) {
      echo(
    ...
    if ((row.tools as Array<Record<string, unknown>>).length) {
      echo("\n### Tools\n");
      for (const tool of row.tools as Array<Record<string, unknown>>) {
        if (seenToolHashes.has(tool.hash as string)) {
          echo(`- **${tool.name}**: \`${(tool.hash as string).slice(0, 7)}\``);
        } else {
          seenToolHashes.add(tool.hash as string);
          const description = ((tool.description as string) || "").replace(
            /\s+$/,
            "",
          );
          const indented = description
            ...
            .join("\n");
          echo(
            `- **${tool.name}**: \`${tool.hash}\`<br>\n${indented}<br>\n    Arguments: \`${dumps(
              (tool.input_schema as Record<string, unknown>).properties,
            )}\``,
          );
        }
      }
    ...
    if ((row.tool_results as Array<Record<string, unknown>>).length) {
      echo("\n### Tool results\n");
      for (const toolResult of row.tool_results as Array<
        Record<string, unknown>
      >) {
        let attachmentsDesc = "";
        for (const attachment of toolResult.attachments as Array<
          Record<string, unknown>
        >) {
          let desc = "";
          if (attachment.type) desc += attachment.type + ": ";
          if (attachment.path) desc += attachment.path;
          else if (attachment.url) desc += attachment.url;
        ...
        echo(
          `- **${toolResult.name}**: \`${toolResult.tool_call_id}\`<br>\n${fencedBlock(
    ...
    let responseText = row.response as string;
    if (row.schema_json) {
      try {
    ...
    if (row.reasoning) {
      echo(`\n## Reasoning\n\n${(row.reasoning as string).replace(/\s+$/, "")}`);
    }
    echo("\n## Response\n");
    if ((row.tool_calls as Array<Record<string, unknown>>).length) {
      echo("### Tool calls\n");
      for (const toolCall of row.tool_calls as Array<Record<string, unknown>>) {
        echo(
          `- **${toolCall.name}**: \`${toolCall.tool_call_id}\`<br>\n${formatToolCallArguments(
            toolCall.arguments,
          )}`,
        );
      }
    ...
    if (usage) {
      const tokenUsage = tokenUsageMarkdown(
        row.input_tokens as number | null,
        row.output_tokens as number | null,
        row.token_details ? JSON.parse(row.token_details as string) : null,
      );
      if (tokenUsage) {
        echo(`## Token usage\n\n${tokenUsage}\n`);
      }
...
modelsGroup.command({
  ...
  handler: async (params) => {
    ...
    const modelsThatHaveShownOptions = new Set<unknown>();
    for (const modelWithAliases of getModelsWithAliases()) {
      if (async_ && !modelWithAliases.async_model) continue;
      if ((query as string[]).length) {
        if (!(query as string[]).every((q) => modelWithAliases.matches(q))) {
          continue;
        }
      }
      if ((model_ids as string[]).length) {
        if (!modelMatchesIdOrAlias(modelWithAliases, model_ids)) {
          continue;
        }
      }
      if (schemas && !modelWithAliases.model.supports_schema) continue;
      if (tools && !modelWithAliases.model.supports_tools) continue;
      echo(
        renderModelWithAliases(modelWithAliases, {
          options,
          async_,
    ...
    if (
      !(query as string[]).length &&
      !options &&
      !schemas &&
      !(model_ids as string[]).length
    ) {
...
modelsGroup.command({
  ...
  handler: async (params) => {
    ...
    try {
      const model = getModel(params.model as string);
...
templatesGroup.command({
  ...
  handler: async (params) => {
    let template: Template;
    try {
      template = await loadTemplate(params.name as string);
    } catch (e) {
      if (e instanceof LoadTemplateError) {
        throw new ClickException(
          `Template '${params.name}' not found or invalid`,
        );
      }
      throw e;
    }
    const data = Object.fromEntries(
      Object.entries(template.modelDump()).filter(
...
templatesGroup.command({
  ...
  handler: async (params) => {
    ...
    click.edit();
    await loadTemplate(params.name as string);
  },
});
...
templatesGroup.command({
  ...
  handler: async () => {
    ...
    for (const [prefix, loader] of Object.entries(getTemplateLoaders())) {
      ...
      const loaderDocs = (loader as { description?: string }).description;
      if (loaderDocs) {
...
schemasGroup.command({
  ...
  handler: async (params) => {
    ...
    let whereSql = "";
    if ((queries as string[]).length) {
      const whereBits = (queries as string[]).map(
        () => "schemas.content like ?",
      );
      whereSql += ` where ${whereBits.join(" and ")}`;
      sqlParams.push(...(queries as string[]).map((q) => `%${q}%`));
    }
    ...
    for (const row of rows) {
      echo(`- id: ${row.id}`);
      if (full) {
        const indented = dumps(JSON.parse(row.content as string), {
          indent: 2,
        })
          .split("\n")
          .map((l) => (l.trim() ? "    " + l : l))
        ...
        echo(
          `  summary: |\n    ${schemaSummary(JSON.parse(row.content as string))}`,
        );
      }
      echo(
        `  usage: |\n    ${row.times_used} time${
          row.times_used !== 1 ? "s" : ""
        }, most recently ${row.recently_used}`,
      );
    }
  },
});
...
schemasGroup.command({
  ...
  handler: async (params) => {
    ...
    echo(dumps(JSON.parse(row.content as string), { indent: 2 }));
  },
});
...
schemasGroup.command({
  ...
  handler: async (params) => {
    const schema = schemaDsl(params.input as string, Boolean(params.multi));
...
function functionSignature(implementation: Function | null): string {
...
toolsGroup.command({
  ...
  handler: async (params) => {
    ...
    function introspectTools(toolboxClass: typeof Toolbox) {
    ...
    let tools: Record<string, Tool | typeof Toolbox | Toolbox>;
    if ((tool_defs as string[]).length) {
      tools = {};
      for (const tool of gatherTools(tool_defs, python_tools)) {
        if (tool instanceof Tool) {
          tools[tool.name] = tool;
        } else {
          tools[(tool as object).constructor.name] = tool as Toolbox;
        }
      }
    } else {
      tools = getTools();
      if ((python_tools as string[]).length) {
        for (const codeOrPath of python_tools as string[]) {
    ...
    const outputTools: Array<Record<string, unknown>> = [];
    const outputToolboxes: Array<Record<string, unknown>> = [];
    const toolObjects: Tool[] = [];
    const toolboxObjects: Array<typeof Toolbox | Toolbox> = [];
    for (const [name, tool] of Object.entries(tools).sort(([a], [b]) =>
      a < b ? -1 : a > b ? 1 : 0,
    )) {
      if (tool instanceof Tool) {
        ...
        toolboxObjects.push(tool as typeof Toolbox);
        const cls =
          typeof tool === "function"
            ? (tool as typeof Toolbox)
            : ((tool as Toolbox).constructor as typeof Toolbox);
    ...
    if (json_) {
      echo(
        dumps(
          { tools: outputTools, toolboxes: outputToolboxes },
          { indent: 2 },
        ),
      );
    } else {
      for (const tool of toolObjects) {
        ...
        echo(
          `${tool.name}${sig}${tool.plugin ? ` (plugin: ${tool.plugin})` : ""}\n`,
        );
        if (tool.description) {
          echo(
            tool.description
      ...
      for (const toolbox of toolboxObjects) {
        const cls =
          typeof toolbox === "function"
            ? (toolbox as typeof Toolbox)
            : (toolbox.constructor as typeof Toolbox);
        echo(cls.name + ":\n");
...
aliasesGroup.command({
  ...
  handler: async (params) => {
    ...
    if (params.json_) {
      echo(
        dumps(Object.fromEntries(toOutput.map(([k, v]) => [k, v])), {
          indent: 4,
        }),
      );
      return;
    ...
    for (const [alias, modelId, type_] of toOutput) {
      echo(
        `${alias.padEnd(maxAliasLength)} : ${modelId}${
          type_ ? ` (${type_})` : ""
        }`,
      );
    }
...
aliasesGroup.command({
  ...
  handler: async (params) => {
    ...
    if (!model_id) {
      ...
      if (!found) {
        throw new ClickException(
          "No model found matching query: " + (query as string[]).join(", "),
        );
      }
...
aliasesGroup.command({
  ...
  handler: async (params) => {
    try {
      removeAlias(params.alias as string);
    } catch (ex) {
      throw new ClickException((ex as Error).message);
    }
  },
...
fragmentsGroup.command({
  ...
  handler: async (params) => {
    ...
    for (const result of results) {
      result.aliases = JSON.parse(result.aliases as string);
    }
    if (json_) {
      echo(dumps(results, { indent: 4 }));
    } else {
      for (const result of results) {
        result.content = truncateString(result.content as string);
        echo(
          pyYamlDump([result], {
            sortKeys: false,
...
fragmentsGroup.command({
  ...
  handler: async (params) => {
    ...
    try {
      resolved = (await resolveFragments(db, [params.fragment as string]))[0] as Fragment;
    } catch (ex) {
      if (ex instanceof FragmentNotFound) {
        throw new ClickException(ex.message);
      }
...
fragmentsGroup.command({
  ...
  handler: async (params) => {
    const db = new Database(logsDbPath());
    migrate(db);
    let resolved: Fragment | Attachment;
    try {
      resolved = (
        await resolveFragments(db, [params.alias_or_hash as string])
      )[0];
    } catch (ex) {
      if (ex instanceof FragmentNotFound) {
        throw new ClickException(ex.message);
...
cli.addCommand(
  new Command({
    ...
    handler: async (params) => {
      ...
      const hooks = new Set(params.hooks as string[]);
      if (hooks.size) {
        plugins = plugins.filter((plugin) =>
          plugin.hooks.some((h) => hooks.has(h)),
        );
      }
...
cli.addCommand(
  new Command({
    ...
    handler: async () => {
      throw new ClickException(
        "llm-ts does not manage plugin installs; use npm install instead",
      );
    },
  }),
);
...
cli.addCommand(
  new Command({
    ...
    handler: async () => {
      throw new ClickException(
        "llm-ts does not manage plugin installs; use npm uninstall instead",
      );
    },
  }),
);
...
cli.addCommand(
  new Command({
    ...
    handler: async (params) => {
      ...
      let content: string | Uint8Array | null = params.content ?? null;
      if (collection && !id) {
      ...
      const getDb = () =>
        new Database(
          database ? String(database) : path.join(userDir(), "embeddings.db"),
        );

      let collectionObj: Collection | null = null;
      ...
      if (!content) {
        if (!input || input === "-") {
          ...
          content = binary
            ? fs.readFileSync(input as string)
            : fs.readFileSync(input as string, "utf-8");
        }
      }
...
function rowsFromFile(
  ...
  if (!detected) {
    ...
    if (trimmed.startsWith("[")) detected = "json";
    else if (trimmed.startsWith("{")) detected = "nl";
    else if (text.includes("\t")) detected = "tsv";
    else detected = "csv";
  }
  if (detected === "json") {
...
cli.addCommand(
  new Command({
    ...
    handler: async (params) => {
      ...
      for (const [alias, attachPath] of attach as Array<[string, string]>) {
      ...
      if ((files as Array<[string, string]>).length) {
        const encodingList = (encodings as string[]).length
          ? (encodings as string[])
          : ["utf-8", "latin-1"];
        rows = [];
        for (const [directory, pattern] of files as Array<[string, string]>) {
          if (
        ...
        rows = db.query(sql as string);
      } else {
        let content: Buffer;
        if (input_path !== "-") {
          content = fs.readFileSync(input_path as string);
        } else {
          content = click.getStreams().readStdinBuffer();
        }
        try {
          rows = rowsFromFile(content, (format as string) ?? null);
        } catch (ex) {
          throw new ClickException((ex as Error).message);
        }
      }

      const tuples: Array<[string, string | Uint8Array]> = [];
      for (const row of rows) {
        const values = Object.values(row);
        const id = String(prefix ?? "") + String(values[0]);
        let content: string | Uint8Array;
        if (binary) {
          content = values[1] as Uint8Array;
        } else {
          content = values
            .slice(1)
      ...
      const embedKwargs: { store: boolean; batch_size?: number } = {
        store: Boolean(store),
      };
      if (batch_size) {
 ...
 */
function decodeStrict(raw: Buffer, encoding: string): string {
...
function globFallback(directory: string, pattern: string): string[] {
...
cli.addCommand(
  new Command({
    ...
    handler: async (params) => {
      const { collection, id, input, binary, number, plain, database, prefix } =
        params;
      let content: string | Uint8Array | null = params.content ?? null;
      if (!id && !content && !input) {
        throw new ClickException(
          "Must provide content or an ID for the comparison",
        );
      }
...
embedModelsGroup.command({
  ...
  handler: async (params) => {
    ...
    for (const modelWithAliases of getEmbeddingModelsWithAliases()) {
      if ((params.query as string[]).length) {
        if (
          !(params.query as string[]).every((q) => modelWithAliases.matches(q))
        ) {
          continue;
        }
      }
...
collectionsGroup.command({
  ...
  handler: async (params) => {
    const database =
      params.database || path.join(userDir(), "embeddings.db");
    ...
    if (params.json_) {
      ...
      for (const row of rows) {
        ...
        echo(
          `  ${row.num_embeddings} embedding${
            row.num_embeddings !== 1 ? "s" : ""
          }`,
        );
      }
    }
...
collectionsGroup.command({
  ...
  handler: async (params) => {
    ...
    let collectionObj: Collection;
    try {
      collectionObj = new Collection(params.collection as string, db, {
        create: false,
      });
    } catch (e) {
      if (e instanceof CollectionDoesNotExist) {
...
optionsGroup.command({
  ...
  handler: async (params) => {
    let modelId: string;
    try {
      modelId = getModel(params.model as string).model_id;
    } catch (e) {
      if (e instanceof UnknownModelError) {
...
optionsGroup.command({
  ...
  handler: async (params) => {
    let modelId: string;
    try {
      const modelObj = getModel(params.model as string);
      modelId = modelObj.model_id;
      try {
        new (modelObj.Options as any)({ [params.key as string]: params.value });
      } catch (ex) {
        if (ex instanceof ValidationError) {
          throw new ClickException(renderErrors(ex.errors()));
        }
    ...
    setModelOption(modelId, params.key as string, params.value);
    echo(
      `Set default option ${params.key}=${params.value} for model ${modelId}`,
      { err: true },
    );
  },
});
...
optionsGroup.command({
  ...
  handler: async (params) => {
    ...
    if (!params.key) {
      ...
      clearedKeys.push(params.key as string);
      clearModelOption(modelId, params.key as string);
    }
    if (clearedKeys.length) {
      if (clearedKeys.length === 1) {
        ...
        echo(
          `Cleared ${clearedKeys.join(", ")} options for model ${modelId}`,
        );
      }
    }
  },
...
loadPlugins();
(pm.hook as any).register_commands({ cli });
