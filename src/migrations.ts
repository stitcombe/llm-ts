...
export type MigrationFn = ((db: Database) => void) & { migrationName?: string };

export const MIGRATIONS: MigrationFn[] = [];

function migration(name: string, fn: (db: Database) => void): void {
  const wrapped = fn as MigrationFn;
...
export function migrate(db: Database): void {
  ...
  const alreadyApplied = new Set(
    db.table("_llm_migrations").rows.map((r) => r.name as string),
  );
...
function pyUtcNowString(): string {
